...
from .AUVDynamics import make_auv_dynamics_block


@sim.declare_block
def make_auv_block(initial_state=np.vstack([0, 0, 0, 0, 0, 0]), linearising_feedback=False):
    # Make sure that the given state is not changed
    state = deepcopy(initial_state)

    # Blocks
    integrate = make_integrator_block(initial_state)
    model = make_auv_dynamics_block()
    if linearising_feedback:
        controller = make_feedback_linearising_controller_block()

    def auv(reference, disturbances=None):
